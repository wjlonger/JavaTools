...
public class Base64 {
	...
	/**
	 * This class need not be instantiated, all methods are constant.
	 *
	 * @since ostermillerutils 1.00.00
	 */
	private Base64(){
	...
	static {
		...
		for (byte i=0; i < ConstUtils.BASE64CHARS.length; i++){
			reverseBase64Chars[ConstUtils.BASE64CHARS[i]] = i;
		}
	 ...
	 * @since ostermillerutils 1.00.00
	 */
	protected static ResourceBundle labels = ResourceBundle.getBundle("com.coder.util.Base64",  Locale.getDefault());
	...
	public static void encode(InputStream in, OutputStream out, boolean lineBreaks) throws IOException {
		...
		while (!done && (inBuffer[0] = in.read()) != END_OF_INPUT){
			...
			out.write(ConstUtils.BASE64CHARS[ inBuffer[0] >> 2 ]);
			if (inBuffer[1] != END_OF_INPUT){
				// B's: last two bits of first byte, first four bits of second byte
				out.write(ConstUtils.BASE64CHARS [(( inBuffer[0] << 4 ) & 0x30) | (inBuffer[1] >> 4) ]);
				if (inBuffer[2] != END_OF_INPUT){
					// C's: last four bits of second byte, first two bits of third byte
					out.write(ConstUtils.BASE64CHARS [((inBuffer[1] << 2) & 0x3c) | (inBuffer[2] >> 6) ]);
					// D's: last six bits of third byte
					out.write(ConstUtils.BASE64CHARS [inBuffer[2] & 0x3F]);
				} else {
					// C's: last four bits of second byte
					out.write(ConstUtils.BASE64CHARS [((inBuffer[1] << 2) & 0x3c)]);
				...
				out.write(ConstUtils.BASE64CHARS [(( inBuffer[0] << 4 ) & 0x30)]);
